...
from fee_structure import COURSES, get_fee_structure, get_all_fee_structures, FEE_INSTALLMENTS, update_fee_structure, reset_fee_structure, reset_all_fee_structures
...
def export_students_csv(course: str = Query(""), db: Session = Depends(get_db)):
    query = db.query(Student)
    if course:
        query = query.filter(Student.course == course)
    ...
    for s in students:
        writer.writerow([getattr(s, col[1], "") or "" for col in EXPORT_COLUMNS])
    output.seek(0)
...
def preview_all_students(course: str = Query(""), db: Session = Depends(get_db)):
    query = db.query(Student)
    if course:
        query = query.filter(Student.course == course)
    students = query.order_by(Student.id.desc()).all()
    rows = []
    for s in students:
        rows.append({col[1]: getattr(s, col[1], "") or "" for col in EXPORT_COLUMNS})
...
@app.put("/api/fee-structure/{course}")
async def update_fee_structure_api(course: str, data: dict):
    """Update fee structure for a specific course."""
    try:
        result = update_fee_structure(course, data)
        return {"message": f"Fee structure updated for {course}", "structure": result}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


@app.post("/api/fee-structure/{course}/reset")
async def reset_fee_structure_api(course: str):
    """Reset fee structure for a specific course back to defaults."""
    reset_fee_structure(course)
    structure = get_fee_structure(course)
    return {"message": f"Fee structure reset for {course}", "structure": structure}


@app.post("/api/fee-structure/reset-all")
async def reset_all_fee_structures_api():
    """Reset ALL fee structures back to defaults."""
    reset_all_fee_structures()
    return {"message": "All fee structures reset to defaults"}
